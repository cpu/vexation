...
async function calculateDirs(store, { extraDirsToCache = [], cachePublic = false }) {
  ...
  const dirsToCache = [cachePublic && resolve(rootDirectory, `public`), resolve(rootDirectory, `.cache`), ...extraDirsToCache.map(dirToCache => resolve(rootDirectory, dirToCache))].filter(Boolean);
...
exports.onPreInit = async function ({ store }, { extraDirsToCache, cachePublic }) {
  ...
  const { dirsToCache, netlifyCacheDir, rootDirectory } = await calculateDirs(store, {
    extraDirsToCache,
    cachePublic
  });
